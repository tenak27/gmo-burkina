...
const COLUMNS = [
  { key: "name", label: "Nom", render: (v, r) => <p className="font-heading text-sm font-bold text-obsidian">{v}</p> },
  { key: "type", label: "Type", render: v => <span className="text-xs px-2.5 py-1 rounded-full bg-gray-100 text-obsidian/60 font-body capitalize">{v || "—"}</span> },
  { key: "phone", label: "Téléphone", render: v => <span className="text-sm text-obsidian/70 font-body">{v || "—"}</span> },
  { key: "city", label: "Ville", render: v => <span className="text-sm text-obsidian/60 font-body">{v || "—"}</span> },
  { key: "credit_limit", label: "Plafond", align: "right", render: v => v ? <span className="text-sm font-heading font-bold text-obsidian">{Number(v).toLocaleString()} FCFA</span> : <span className="text-obsidian/25">—</span> },
  { key: "is_active", label: "Statut", align: "center", render: v => <span className={`text-xs px-3 py-1 rounded-full font-semibold font-body ${v !== false ? "bg-green-100 text-green-700 border border-green-200" : "bg-red-50 text-red-600 border border-red-200"}`}>{v !== false ? "Actif" : "Inactif"}</span> },
];
