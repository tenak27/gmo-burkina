...
const COLUMNS = [
  { key: "name", label: "Produit", render: (v, r) => <div><p className="font-heading text-xs font-semibold text-obsidian">{v}</p><span className="text-[10px] text-obsidian/40 font-body capitalize">{r.category || "—"}</span></div> },
  { key: "unit", label: "Unité", render: v => <span className="text-xs text-obsidian/50 font-body">{v || "—"}</span> },
  { key: "unit_price", label: "Prix U.", align: "right", render: v => v ? <span className="text-xs font-heading font-bold text-obsidian">{Number(v).toLocaleString()}</span> : <span className="text-obsidian/25">—</span> },
  { key: "wholesale_price", label: "Prix Gros", align: "right", render: v => v ? <span className="text-xs font-heading font-bold text-gmo-green">{Number(v).toLocaleString()}</span> : <span className="text-obsidian/25">—</span> },
  { key: "stock_quantity", label: "Stock", align: "right", render: (v, r) => {
    const low = v <= (r.stock_alert || 10);
    return <span className={`font-heading text-sm font-bold ${v === 0 ? "text-red-500" : low ? "text-amber-500" : "text-obsidian"}`}>{v ?? "—"}</span>;
  }},
  { key: "is_active", label: "Actif", align: "center", render: v => <span className={`text-[10px] px-2 py-0.5 rounded-full font-body ${v !== false ? "bg-green-50 text-green-700 border border-green-200" : "bg-red-50 text-red-600 border border-red-200"}`}>{v !== false ? "Oui" : "Non"}</span> },
];
